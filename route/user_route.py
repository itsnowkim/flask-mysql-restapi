...
def select_user(name):
    ...
    if len(select_user) == 0:
        ...
        return dict(id = select_user.id, name = select_user.user_name, created_at = select_user.created_at, updated_at = select_user.updated_at)
...
def select_all_user():
    ...
    if len(select_user) == 0:
        ...
        for user in select_user:
            data = dict(id = user.id, name = user.user_name, created_at = user.created_at, updated_at = user.updated_at)
...
def update_user(name):
    ...
    if len(select_user) == 0:
        ...
        user.updated_at = datetime.datetime.now().replace(tzinfo=None)
        my_user.db.session.commit()
        my_user.db.session.remove()
        return "update!"
